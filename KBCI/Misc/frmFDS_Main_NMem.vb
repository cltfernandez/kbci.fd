...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.ToolBar1.Size = New System.Drawing.Size(776, 46)
        ...
        Me.GroupBox1.Size = New System.Drawing.Size(768, 552)
        ...
        Me.Label51.Location = New System.Drawing.Point(16, 528)
        ...
        Me.Label33.Location = New System.Drawing.Point(120, 528)
        Me.Label33.Name = "Label33"
        Me.Label33.Size = New System.Drawing.Size(632, 16)
        Me.Label33.TabIndex = 41
        Me.Label33.Text = "Label10"
        ...
        Me.ComboBox4.Items.AddRange(New Object() {"SERVICE MASTER"})
        Me.ComboBox4.Location = New System.Drawing.Point(96, 64)
        Me.ComboBox4.Name = "ComboBox4"
        Me.ComboBox4.Size = New System.Drawing.Size(280, 21)
        ...
        Me.ComboBox6.Items.AddRange(New Object() {"MAINTENANCE"})
        ...
        Me.GroupBox5.Location = New System.Drawing.Point(16, 400)
        ...
        Me.ClientSize = New System.Drawing.Size(776, 613)
...
#End Region
    ...
    Function KBCINumber() As String
        ...
        Dim NOKBCI As String = Microsoft.VisualBasic.Right(CStr(rsCTL.Fields("SYSDATE").Value), 2)
        Dim INS As String
        Dim i As Integer = 7
    ...
    Private Sub ToolBar1_ButtonClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs) Handles ToolBar1.ButtonClick
        ...
        With ToolBar1
            Select Case IDX
                        ...
                        If valTXT() Then
                            ...
                            If MSG = vbYes Then
                                ...
                                "VALUES    ('" & KBCINUM & "','" & TextBox1.Text.ToUpper & "','" & TextBox4.Text.ToUpper & "','" & TextBox5.Text.ToUpper & "','" & Mid(ComboBox3.Text.ToUpper, 1, 1) & "','A','" & rsCTL.Fields("SYSDATE").Value & "','" & CLUSER & "','" & TextBox10.Text.ToUpper & "','" & DateValue(DateTimePicker2.Value.ToString) & "','" & ComboBox6.Text.ToUpper & "','" & TextBox16.Text.ToUpper & "','" & Trim(MaskEdBox2.CtlText.ToUpper.Replace("-", "") & AxMaskEdBox3.CtlText) & "','" & CheckBox1.Checked.ToString & "','" & TextBox13.Text.ToUpper & "','" & Mid(ComboBox1.Text.ToUpper, 1, 1) & "','" & DateValue(DateTimePicker1.Value.ToString) & "','" & Mid(ComboBox2.Text.ToUpper, 1, 1) & "','" & TextBox2.Text.ToUpper & "','" & Trim(MaskEdBox1.CtlText.Replace("-", "")) & "','" & _
                                                ...
                                                ",0.00,0.00,0.00,'" & rsCTL.Fields("SYSDATE").Value & "','" & rsCTL.Fields("SYSDATE").Value & "','" & CLUSER & "','False',0)")
                                cn.Execute("INSERT INTO FD([KBCI_NO],[TRAN_CODE],[DATE],[REF],[AMOUNT],[BALANCE],[RMK],[ADD_DATE],[USER] ,[LPOSTED],[DRCR],[BANK_CODE],[CHECKNO],[TPOSTED],[TREVERSED])" & _
                                         "VALUES('" & KBCINUM & "','1','" & rsCTL.Fields("SYSDATE").Value & "','INITIAL'," & Decimal.Round(TextBox25.Text, 2) & "," & Decimal.Round(TextBox25.Text, 2) & ",'INITIAL SETUP','" & rsCTL.Fields("SYSDATE").Value & "','" & CLUSER & "',0,'CR','','','True','False')")
                        ...
                        If valTXT() Then
                            ...
                            If MSG = vbYes Then
                                ...
                                cn.Execute("UPDATE MEMBERS SET [LNAME]='" & TextBox1.Text.ToUpper & "',[FNAME]='" & TextBox4.Text.ToUpper & "',[MI]='" & TextBox5.Text.ToUpper & "',[MEM_CODE]='" & Mid(ComboBox3.Text.ToUpper, 1, 1) & _
                                                            ...
                                                            "',[SP_CBEMPNO]='" & TextBox21.Text.ToUpper & "',[SP_KBCINO]='" & TextBox18.Text.ToUpper & "',[SP_SALARY]=" & Decimal.Round(TextBox22.Text, 2) & ",[CHG_DATE]='" & rsCTL.Fields("SYSDATE").Value & "',[USER]='" & CLUSER & "',[REC_STAT]='False',[FD_CNTR]=0 WHERE KBCI_NO ='" & TextBox3.Text.Replace("-", "") & "'")
                                .Buttons(8).Text = "EXIT"
                                rsMEMBERS.Requery() : rsMEMBERS.MoveFirst()
                            End If
                        Else
                ...
                Case 2
                        MsgBox(Val(ToolBar1.Buttons.IndexOf(e.Button).ToString))
                Case 4
                    Dim frmFDS_Main_PrntFDL_Srch As New frmFDS_Main_PrntFDL_Srch
                    frmFDS_Main_PrntFDL_Srch.ShowDialog()
                    If SW = True Then
                        rsMEMBERS.MoveFirst()
                        rsMEMBERS.Find("KBCI_NO=" & frmFDS_Main_PrntFDL_Srch.TextBox3.Text.Trim.Replace("-", ""))
                        FillField()
                    End If
                Case 5
                    If Not rsMEMBERS.BOF Then rsMEMBERS.MovePrevious() : FillField()
                Case 6
                    If Not rsMEMBERS.EOF Then rsMEMBERS.MoveNext() : FillField()
                Case 8
    ...
    Private Sub frmFDS_Main_NMem_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        rsMEMBERS.Open("SELECT * FROM Members ", cn, CursorTypeEnum.adOpenKeyset, LockTypeEnum.adLockOptimistic)
        rsMEMBERS.MoveFirst()
        FillField()
    End Sub
    ...
    Private Sub TextBox12_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox12.LostFocus
        If Trim(TextBox12.Text) <> "" Then TextBox12.Text = ValAcct(TextBox12)
    End Sub
    Function ValAcct(ByRef TXT As TextBox) As String
        Dim v, v2, i, chk As Integer
        Dim tot As Integer = 0
        Dim s As String
        i = 10
        v = Val(Mid(TXT.Text, 3, 2))
        v2 = Microsoft.VisualBasic.Right(Trim(TXT.Text), 6)
        s = Microsoft.VisualBasic.Left(Trim(TXT.Text), 2)
        If IsNumeric(TextBox12.Text) And Len(Trim(TXT.Text)) = 10 And (s = "00" Or s = "01" Or s = "51" Or s = "50") Then
            ...
            Else
                Do Until i <= 1
                    tot = tot + Val(Mid(Trim(TXT.Text), 11 - i, 1)) * i
                    i -= 1
                Loop
                If (chk = tot Mod 11) > 1 Then MsgBox("INVALID CHECK DIGIT", MsgBoxStyle.Information, "FEBTC S/A - C/A Number")
            End If
        Else
            MsgBox("INVALID ACCOUNT No.", MsgBoxStyle.Information, "FEBTC S/A - C/A Number") : Exit Function
        End If
        s = Mid(TXT.Text, 1, 4) & "-" & Mid(TXT.Text, 5, 5) & "-" & Mid(TXT.Text, 10, 1) : Exit Function
        Return s
    End Function
    ...
    Private Sub TextBox14_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox14.LostFocus
        If Trim(TextBox14.Text) <> "" Then TextBox14.Text = ValAcct(TextBox14)
    End Sub
    ...
    Private Sub TextBox22_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox22.LostFocus
        TextBox22.Text = FormatNumber(Val(TextBox22.Text), 2)
    End Sub
    Private Sub TextBox25_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox25.LostFocus
        TextBox25.Text = FormatNumber(Val(TextBox25.Text), 2)
    End Sub
    Private Sub TextBox23_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox23.LostFocus
        TextBox23.Text = FormatNumber(Val(TextBox23.Text), 2)
    End Sub
    Sub FillField()
        On Error Resume Next
        With rsMEMBERS
            If Not IsDBNull(.Fields("KBCI_NO").Value) Then TextBox3.Text = Mid(.Fields("KBCI_NO").Value, 1, 2) & "-" & Mid(.Fields("KBCI_NO").Value, 3, 4) & "-" & Mid(.Fields("KBCI_NO").Value, 7, 1)
            If Not IsDBNull(.Fields("LNAME").Value) Then TextBox1.Text = .Fields("LNAME").Value
            If Not IsDBNull(.Fields("FNAME").Value) Then TextBox4.Text = .Fields("FNAME").Value
            If Not IsDBNull(.Fields("MI").Value) Then TextBox5.Text = .Fields("MI").Value
            If Not IsDBNull(.Fields("MEM_CODE").Value) Then ComboBox3.Text = .Fields("MEM_CODE").Value
            If Not IsDBNull(.Fields("CB_EMPNO").Value) Then TextBox10.Text = .Fields("CB_EMPNO").Value
            If Not IsDBNull(.Fields("CB_HIRE").Value) Then DateTimePicker2.Value = DateValue(.Fields("CB_HIRE").Value)
            If Not IsDBNull(.Fields("DEPT").Value) Then ComboBox6.Text = .Fields("DEPT").Value
            If Not IsDBNull(.Fields("Region").Value) Then TextBox16.Text = .Fields("Region").Value
            If Not IsDBNull(.Fields("OFF_TEL").Value) Then MaskEdBox2.CtlText = Mid(.Fields("OFF_TEL").Value, 1, 3) & "-" & Mid(.Fields("OFF_TEL").Value, 4, 4)
            If Not IsDBNull(.Fields("OFF_TEL").Value) Then AxMaskEdBox3.CtlText = Mid(.Fields("OFF_TEL").Value, 8, 4)
            If Not IsDBNull(.Fields("DORI").Value) Then CheckBox1.Checked = CBool(.Fields("DORI").Value)
            If Not IsDBNull(.Fields("REA_DORI").Value) Then TextBox13.Text = .Fields("REA_DORI").Value
            If Not IsDBNull(.Fields("SEX").Value) Then ComboBox1.Text = .Fields("SEX").Value
            If Not IsDBNull(.Fields("B_DATE").Value) Then DateTimePicker1.Value = DateValue(.Fields("B_DATE").Value)
            If Not IsDBNull(.Fields("CIV_STAT").Value) Then ComboBox2.Text = .Fields("CIV_STAT").Value
            If Not IsDBNull(.Fields("MEM_ADDR").Value) Then TextBox2.Text = .Fields("MEM_ADDR").Value

            If Not IsDBNull(.Fields("RES_TEL").Value) Then MaskEdBox1.CtlText = Mid(.Fields("RES_TEL").Value, 1, 3) & "-" & Mid(.Fields("RES_TEL").Value, 4, 4)
            If Not IsDBNull(.Fields("POSITION").Value) Then ComboBox4.Text = .Fields("POSITION").Value
            If Not IsDBNull(.Fields("SAL_BAS").Value) Then TextBox15.Text = FormatNumber(.Fields("SAL_BAS").Value, 2)
            If Not IsDBNull(.Fields("SAL_ALL").Value) Then TextBox17.Text = FormatNumber(.Fields("SAL_ALL").Value, 2)

            If Not IsDBNull(.Fields("OTH_INC").Value) Then TextBox28.Text = FormatNumber(.Fields("OTH_INC").Value, 2)
            If Not IsDBNull(.Fields("FEBTC_SA").Value) Then TextBox12.Text = Mid(.Fields("FEBTC_SA").Value, 1, 4) & "-" & Mid(.Fields("FEBTC_SA").Value, 5, 5) & "-" & Mid(.Fields("FEBTC_SA").Value, 10, 1)
            If Not IsDBNull(.Fields("FEBTC_CA").Value) Then TextBox14.Text = Mid(.Fields("FEBTC_CA").Value, 1, 4) & "-" & Mid(.Fields("FEBTC_CA").Value, 5, 5) & "-" & Mid(.Fields("FEBTC_CA").Value, 10, 1)

            If Not IsDBNull(.Fields("REM_VALUE").Value) Then TextBox7.Text = FormatNumber(.Fields("REM_VALUE").Value, 2)
            'AxMaskEdBox1.CtlText = Val(.Fields("NO_DEPEND").Value).ToString("D2")
            If Not IsDBNull(.Fields("SP_NAME").Value) Then TextBox19.Text = .Fields("SP_NAME").Value
            If Not IsDBNull(.Fields("SP_EMPLOY").Value) Then TextBox20.Text = .Fields("SP_EMPLOY").Value
            If Not IsDBNull(.Fields("SP_OFFTEL").Value) Then AxMaskEdBox2.CtlText = Mid(.Fields("SP_OFFTEL").Value, 1, 3) & "-" & Mid(.Fields("SP_OFFTEL").Value, 4, 4)
            'AxMaskEdBox4.CtlText = Mid(.Fields("SP_OFFTEL").Value, 8, 4)
            If Not IsDBNull(.Fields("SP_CBEMPNO").Value) Then TextBox21.Text = .Fields("SP_CBEMPNO").Value
            If Not IsDBNull(.Fields("SP_KBCINO").Value) Then TextBox18.Text = .Fields("SP_KBCINO").Value
            ...
            If Not IsDBNull(.Fields("FD_AMOUNT").Value) Then TextBox25.Text = FormatNumber(.Fields("FD_AMOUNT").Value, 2)
            If Not IsDBNull(.Fields("SD_AMOUNT").Value) Then TextBox23.Text = FormatNumber(.Fields("SD_AMOUNT").Value, 2)
        End With
    End Sub
